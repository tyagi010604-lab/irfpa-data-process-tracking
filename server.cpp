/*
 * IRFPA Management System — C++ HTTP Server
 * ==========================================
 * Self-contained: no external headers beyond C++ stdlib + POSIX sockets.
 * Uses inline SQLite3 declarations (links against libsqlite3.so) and
 * a built-in SHA-256 / HMAC-SHA-256 implementation (no OpenSSL needed).
 *
 * Compile (Linux):
 *   g++ -std=c++17 -O2 -o server server.cpp -lsqlite3 -lpthread
 *
 * Compile (macOS):
 *   g++ -std=c++17 -O2 -o server server.cpp -lsqlite3 -lpthread
 *
...
// We declare only what we use so no header is required.
// Link with: -lsqlite3
extern "C" {
  struct sqlite3;
  struct sqlite3_stmt;
...
// ─────────────────────────────────────────────────────────────────────────────
// SHA-256 (public domain implementation, RFC 6234 compatible)
// ─────────────────────────────────────────────────────────────────────────────
...
std::string hmac_sha256(const std::string& key, const std::string& msg) {
  ...
  std::string opad(B, 0x5c), ipad(B, 0x36);
...
std::string verify_jwt(const std::string& token) {
  ...
  long long exp = std::stoll(pj.substr(ep+6));
  if (now_epoch() > exp) return "";
  // extract sub
  auto sp = pj.find("\"sub\":\""); if (sp==std::string::npos) return "";
  sp += 7;
  auto se = pj.find('"', sp);     if (se==std::string::npos) return "";
...
void init_db() {
  ...
  auto add_user = [&](const char* u, const char* p, const char* role, const char* stage) {
    ...
    std::string sql =
      "INSERT OR IGNORE INTO users (username,password_hash,role,assigned_stage) VALUES ('" +
      std::string(u) + "','" + h + "','" + role + "'," +
      (stage ? ("'" + std::string(stage) + "'") : "NULL") + ");";
...
struct Req {
  ...
  std::string bearer() const {
    ...
    if (it->second.size() > 7 && it->second.substr(0,7) == "Bearer ")
...
bool authenticate(const Req& req, std::map<std::string,std::string>& usr) {
  std::string tok = req.bearer();
  if (tok.empty()) return false;
  std::string uname = verify_jwt(tok);
  ...
  if (sqlite3_step(s) == SQLITE_ROW) {
    ...
    auto col = [&](int i) -> std::string {
      auto v = sqlite3_column_text(s, i); return v ? (const char*)v : "";
    };
    usr["id"]             = std::to_string(sqlite3_column_int(s, 0));
    usr["username"]       = col(1);
    usr["role"]           = col(2);
    usr["assigned_stage"] = col(3);
    usr["last_login"]     = col(4);
  }
...
Res h_login(const Req& req) {
  auto uname = json_str(req.body, "username");
  auto pass  = json_str(req.body, "password");
  ...
  if (sqlite3_step(s) != SQLITE_ROW) {
    sqlite3_finalize(s); sqlite3_close(db);
    return Res::err("Invalid credentials", 401);
  }
  ...
  std::string upd = "UPDATE users SET last_login=datetime('now') WHERE id="+std::to_string(uid);
  sqlite3_exec(db, upd.c_str(), nullptr, nullptr, nullptr);
  std::string alog = "INSERT INTO audit_logs(user_id,action,table_name) VALUES("
                     +std::to_string(uid)+",'LOGIN','users')";
  sqlite3_exec(db, alog.c_str(), nullptr, nullptr, nullptr);
  sqlite3_close(db);
  ...
  return Res::json("{\"access_token\":\""+tok+"\","
                   ...
                   ",\"username\":\""+json_escape(un)+"\""
                   ",\"role\":\""+role+"\""
                   ",\"assigned_stage\":\""+json_escape(astage)+"\"}}");
}
...
Res h_me(const Req& req) {
  ...
  return Res::json("{\"id\":"+u["id"]+
                   ...
                   ",\"role\":\""+u["role"]+"\""
                   ",\"assigned_stage\":\""+json_escape(u["assigned_stage"])+"\""
...
Res h_stages(const Req& req) {
  ...
  while (sqlite3_step(s) == SQLITE_ROW) {
    ...
    r += "{\"id\":\""+json_escape(c(0))+"\",\"name\":\""+json_escape(c(1))+
         "\",\"description\":\""+json_escape(c(2))+"\"}";
  }
...
Res h_create_entry(const Req& req) {
  ...
  auto stage_id     = json_str(req.body, "stage_id");
  auto element_name = json_str(req.body, "element_name");
  auto value        = json_str(req.body, "value");
  auto unit         = json_str(req.body, "unit");
  auto device       = json_str(req.body, "device");
  auto notes        = json_str(req.body, "notes");
  auto image_data   = json_str(req.body, "image_data");
  auto image_name   = json_str(req.body, "image_name");

  if (stage_id.empty() || element_name.empty() || value.empty())
    return Res::err("Missing required fields", 400);
  if (u["role"] != "admin" && u["assigned_stage"] != stage_id)
    return Res::err("Access denied to this stage", 403);
  ...
  // Audit log
  std::string alog_sql =
    "INSERT INTO audit_logs(user_id,action,table_name,record_id,stage_id,element_name,new_value)"
    " VALUES("+u["id"]+",'CREATE','data_entries',"+std::to_string(eid)+
    ",'"+stage_id+"','"+element_name+"','"+value+"')";
  sqlite3_exec(db, alog_sql.c_str(), nullptr, nullptr, nullptr);

  // Auto-archive: check if all 7 processing stages completed for this device
  static const std::vector<std::string> PROC = {
    "czt_substrate","epilayer","fabrication","measurement","hybridization","assembly","testing"
  };
  if (!device.empty()) {
    sqlite3_stmt* cs;
    sqlite3_prepare_v2(db,
  ...
  sqlite3_close(db);
  return Res::json("{\"id\":"+std::to_string(eid)+
                   ",\"stage_id\":\""+json_escape(stage_id)+"\""
                   ",\"element_name\":\""+json_escape(element_name)+"\""
                   ",\"value\":\""+json_escape(value)+"\""
                   ",\"unit\":\""+json_escape(unit)+"\""
                   ",\"device\":\""+json_escape(device)+"\""
                   ",\"notes\":\""+json_escape(notes)+"\""
                   ",\"has_image\":"+(!image_data.empty()?"true":"false")+
                   ",\"image_name\":\""+json_escape(image_name)+"\""
                   ",\"created_by\":\""+json_escape(u["username"])+"\""
                   ",\"created_at\":\""+now_string()+"\""
                   ",\"updated_at\":\""+now_string()+"\"}", 201);
}
...
Res h_get_entries(const Req& req) {
  ...
  std::string df = p.count("device")     ? p.at("device")     : "";
  std::string pf = p.count("parameters") ? p.at("parameters") : "";
  ...
  if (u["role"] != "admin")
    sql += " AND de.stage_id='" + u["assigned_stage"] + "'";
  else if (!sf.empty())
    sql += " AND de.stage_id='" + sf + "'";

  if (!df.empty()) sql += " AND de.device='" + df + "'";

  if (!pf.empty()) {
    std::vector<std::string> pns;
    std::istringstream pss(pf); std::string pp;
  ...
  while (sqlite3_step(s) == SQLITE_ROW) {
    ...
    auto c = [&](int i) -> std::string { auto v=sqlite3_column_text(s,i); return v?(const char*)v:""; };
    r += "{\"id\":"+c(0)+
         ",\"stage_id\":\""+json_escape(c(1))+"\""
         ",\"element_name\":\""+json_escape(c(2))+"\""
         ",\"value\":\""+json_escape(c(3))+"\""
         ",\"unit\":\""+json_escape(c(4))+"\""
         ",\"device\":\""+json_escape(c(5))+"\""
         ",\"notes\":\""+json_escape(c(6))+"\""
         ",\"has_image\":" + (sqlite3_column_int(s,7)?"true":"false") +
         ",\"image_name\":\""+json_escape(c(8))+"\""
         ",\"created_by\":\""+json_escape(c(9))+"\""
         ",\"created_at\":\""+json_escape(c(10))+"\""
         ",\"updated_at\":\""+json_escape(c(11))+"\"}";
  }
  r += "]"; sqlite3_finalize(s); sqlite3_close(db);
  return Res::json(r);
}

// GET /api/data-entries/:id/image
...
Res h_audit_logs(const Req& req) {
  ...
  while (sqlite3_step(s) == SQLITE_ROW) {
    ...
    auto c=[&](int i)->std::string{auto v=sqlite3_column_text(s,i);return v?(const char*)v:"";};
    r += "{\"id\":"+c(0)+
         ",\"username\":\""+json_escape(c(1))+"\""
         ",\"action\":\""+json_escape(c(2))+"\""
         ",\"table_name\":\""+json_escape(c(3))+"\""
         ",\"stage_id\":\""+json_escape(c(4))+"\""
         ",\"element_name\":\""+json_escape(c(5))+"\""
         ",\"old_value\":\""+json_escape(c(6))+"\""
         ",\"new_value\":\""+json_escape(c(7))+"\""
         ",\"timestamp\":\""+json_escape(c(8))+"\"}";
  }
  r += "]"; sqlite3_finalize(s); sqlite3_close(db);
  return Res::json(r);
}
...
Res h_export_audit(const Req& req) {
  ...
  sqlite3_stmt* s;
  sqlite3_prepare_v2(db,
    "SELECT al.timestamp,u.username,al.action,al.stage_id,"
    "al.element_name,al.old_value,al.new_value"
...
Res h_get_users(const Req& req) {
  ...
  while (sqlite3_step(s) == SQLITE_ROW) {
    ...
    r += "{\"id\":"+std::to_string(sqlite3_column_int(s,0))+
         ",\"username\":\""+json_escape(c(1))+"\""
         ",\"role\":\""+c(2)+"\""
         ",\"assigned_stage\":\""+json_escape(c(3))+"\""
         ",\"last_login\":\""+json_escape(c(4))+"\""
...
Res h_toggle_user(const Req& req, const std::string& id_str) {
  ...
  sqlite3_bind_int(s, 1, std::stoi(id_str));
  if (sqlite3_step(s) != SQLITE_ROW) {
    sqlite3_finalize(s); sqlite3_close(db); return Res::err("User not found", 404);
  }
  ...
  std::string alog = "INSERT INTO audit_logs(user_id,action,table_name,element_name,old_value,new_value)"
    " VALUES("+admin["id"]+",'"+action+"','users','"+tuser+"','"
    +(old_active?"Active":"Inactive")+"','"+(new_active?"Active":"Inactive")+"')";
  sqlite3_exec(db, alog.c_str(), nullptr, nullptr, nullptr);
  sqlite3_close(db);
...
Res h_get_devices(const Req& req) {
  ...
  while (sqlite3_step(s)==SQLITE_ROW) {
    auto v=sqlite3_column_text(s,0); if(v) devs.push_back((const char*)v);
  }
  sqlite3_finalize(s);
  std::string r = "["; bool first = true;
  for (auto& dev : devs) {
    ...
    int archived = (sqlite3_step(cs)==SQLITE_ROW)?sqlite3_column_int(cs,0):0;
    sqlite3_finalize(cs);
    int pct = (int)(100.0*completed/7);
    r += "{\"device\":\""+json_escape(dev)+"\""
         ",\"completed_stages\":"+std::to_string(completed)+
         ",\"total_stages\":7"
         ",\"completion_pct\":"+std::to_string(pct)+
...
Res serve_frontend() {
  ...
  if (!f.good()) return Res::html("<h1>index.html not found — run server from the project directory</h1>");
...
Res route(const Req& req) {
  ...
  // Dynamic: /api/data-entries/:id/image
...
int main() {
  ...
  if (listen(srv,128)<0) { std::cerr<<"listen() failed\n"; return 1; }
